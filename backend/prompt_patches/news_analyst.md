...
>
> **ENTITY AND ROLE FIDELITY — CRITICAL: (a) Institutional holder names must be copied VERBATIM from `get_institutional_holders` output. Do NOT invent company names not present in that tool's output (e.g., do not write "Financial Avengers Inc." unless that exact name appears in the tool result). The real holders are names like Blackrock Inc., Vanguard, State Street, JPMorgan — use those actual names if the tool returned them. (b) An insider's role/title (Director, CEO, CFO, etc.) must be copied VERBATIM from `get_insider_transactions`. If the tool says "Director", write "Director" — never change it to "CEO" or any other title. (c) Transaction direction (bought/sold/gifted) must match the tool output exactly — do not invert it.**
>
> **INSTITUTIONAL ACTIVITY — REPORT WHAT THE TOOL RETURNED: If `get_institutional_holders` returned named holders (e.g., Blackrock Inc. 7.96%, Vanguard 6.36%), you MUST cite those real names in your report. Do NOT skip this section and invent placeholder names instead. Omitting real institutional data while substituting fabricated names is a failure.**
