...
>
> **CURRENCY SYMBOLS: Check the `Reporting currency` line in the get_fundamentals output. If it says EUR, every revenue, income, balance-sheet, and cash-flow figure in your report MUST use the € symbol. The $ symbol is ONLY for NYSE per-share price data. "Revenue (TTM): $20.00B" is wrong for a EUR-reporting company — write "€20.00B".**
>
...
>
> **PRICE/BOOK — METHODOLOGY DISCLOSURE: `get_fundamentals` returns a P/B snapshot that may use a different equity denominator than the Common Stock Equity on the balance sheet (e.g., total equity vs. common equity, or a different as-of date). If you cite both the P/B ratio and specific equity/market-cap figures, note which calculation produced the snapshot ratio — do not imply the snapshot P/B is directly derived from the balance sheet numbers shown in the same sentence if the math doesn't reconcile.**
>
> **FREE CASH FLOW — TWO SOURCES, REPORT BOTH, NO ADJUSTMENTS: Report the yfinance fiscal TTM figure from `get_fundamentals` and the 4-quarter sum from `get_cashflow` SEPARATELY. The 4-quarter sum must equal the EXACT arithmetic of the four quarterly values you cite — do NOT adjust any quarter to make the sum match the TTM figure. If Q2 2025 is 88M in the tool output, report 88M; the sum is 629+227+429+88 = 1,373M. If the two totals disagree, flag the discrepancy: "Note: 4-quarter sum (1,373M) differs from yfinance fiscal TTM (1,600M) — this reflects different calculation windows, not an error." The word "adjusted" must never appear in the FCF section. Inventing a revised quarterly figure to reconcile two source totals is a fabrication.**
